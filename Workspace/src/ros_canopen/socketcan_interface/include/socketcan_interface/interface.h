...
struct Header {
  ...
  unsigned int
      id : 29; ///< CAN ID (11 or 29 bits valid, depending on is_extended member
  ...
  bool isValid() { return id < (is_extended ? (1 << 29) : (1 << 11)); }
  ...
  Header(unsigned int i, bool extended, bool rtr, bool error)
      : id(i), is_error(error ? 1 : 0), is_rtr(rtr ? 1 : 0),
...
struct Frame : public Header {
  boost::array<unsigned char, 8>
      data;          ///< array for 8 data bytes with bounds checking
  unsigned char dlc; ///< len of data

  /** check if frame header and length are valid*/
  bool isValid() { return (dlc <= 8) && Header::isValid(); }
  /**
   * constructor with default parameters
   * @param[in] i: CAN id, defaults to 0
...
public:
   ...
   */
  virtual StateListener::Ptr
  createStateListener(const StateDelegate &delegate) = 0;
...
public:
   ...
   * @return managed pointer to listener
   */
  virtual FrameListener::Ptr
  createMsgListener(const FrameDelegate &delegate) = 0;
   ...
   */
  virtual FrameListener::Ptr
  createMsgListener(const Frame::Header &, const FrameDelegate &delegate) = 0;
