...
CAutonomyTwistMux::CAutonomyTwistMux(ros::NodeHandle &nh)
    : m_pBallFallowVel(nullptr), m_pSpiralVel(nullptr),
      m_pLocalPlannerVel(nullptr), m_pUnstuckVel(nullptr),
      m_pEStopVel(nullptr) {
  // Create publisher
  m_pCmdVelPub =
  ...
  m_pUnstuckSub = new ros::Subscriber(nh.subscribe(
      strUnstuckTopic, 1, &CAutonomyTwistMux::UnstuckCallback, this));
...
void CAutonomyTwistMux::Arbitrate(eAutonomyState state) {
  ...
  case eAutonomyState::BACKTRACK:
    ...
    } else if (m_pUnstuckVel &&
               (TimeSinceMessage(m_UnstuckRecTime) < EXPIRY_TIME)) {
