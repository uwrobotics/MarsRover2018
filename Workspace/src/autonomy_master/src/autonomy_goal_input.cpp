...
void CurGpsCallback(sensor_msgs::NavSatFixConstPtr pGps)
{
  pCurGps = pGps;
}
...
sensor_msgs::NavSatFix CalculateGoal(double bearing, double distance)
{
  ...
  double DestLat = asin(sin(CurLat)*cos(distance/Eradius)+cos(CurLat)*sin(distance/Eradius)*cos(bearing));
  double DestLon = CurLon + atan2(sin(bearing)*sin(distance/Eradius)*cos(CurLat),cos(distance/Eradius)-sin(CurLat)*sin(DestLat));
...
int main(int argc, char *argv[]) {
  ...
  ros::Publisher goalPub = nh.advertise<sensor_msgs::NavSatFix>("/autonomy/goal_gps",1);
