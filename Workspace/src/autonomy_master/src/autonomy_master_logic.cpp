...
CAutonomyMasterLogic::CAutonomyMasterLogic(ros::NodeHandle &nh)
    : m_state(eAutonomyState::IDLE), m_pGoalGps(nullptr),
  ...
  m_pBacktrackGpsSub = new ros::Subscriber(
      nh.subscribe(strBacktrackGpsTopic, 1,
                   &CAutonomyMasterLogic::BacktrackGpsCallback, this));
...
void CAutonomyMasterLogic::GoalGpsCallback(
    sensor_msgs::NavSatFixConstPtr pGoalGps) {
  m_pGoalGps = pGoalGps;
