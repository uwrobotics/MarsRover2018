...
float oGridDataAccessor(occupancy_grid::OccupancyGrid::ConstPtr &oGrid,
...
void odbg(double x, double y, int xi, int yi,
  ...
  // height=%f",x,y,xi,yi,oGridDataAccessor(grid,yi,xi,1));
}

eTraversableResult
IsPointTraversable(occupancy_grid::OccupancyGrid::ConstPtr &pGrid, double y,
                   double x) {
  ...
  if (zInGrid < 0 || xInGrid < 0 ||
      xInGrid >= (int)pGrid->dataDimension[1].size) {
...
double CalcDistance(occupancy_grid::OccupancyGrid::ConstPtr &pGrid, float v,
                    float w, float robotLength, float robotWidth,
                    float timestep, bool &foundDanger) {
  // ROS_INFO("v=%f,w=%f",v,w);
  //        double radius = ((double) v) / w;
  double retDist = 0;
  ///////////
  //        function distance = GetDistScore(OccupancyGrid, velocity, robotDims,
  //        resolution, timestep)
  //        x = 0;%m
  //                y = 0;%m
  //                distanceMax=10;
  ...
  if (std::round(v * 1000) == 0) {
    ...
    //             PointForCoord(pGrid, 0/*robotLength / 2*/, 0, yIndex,
    //             xIndexCenter);
    //             PointForCoord(pGrid, 0/*robotLength / 2*/, bufferFromCenter,
    //             yIndex, xIndexLeftCorn);
    //             PointForCoord(pGrid, 0/*robotLength / 2*/, -bufferFromCenter,
    //             yIndex, xIndexRightCorn);

    // //        xIndexRightCorn = ceil(xIndexCenter +
    // bufferFromCenter/resolution);
    // //        xIndexLeftCorn = floor(xIndexCenter -
    // bufferFromCenter/resolution);
    // //        if ((OccupancyGrid(yIndex, xIndexCenter) == 0) ||
    // (OccupancyGrid(yIndex, xIndexRightCorn) == 0) || (OccupancyGrid(yIndex,
    // xIndexLeftCorn) == 0))
    // //            distance = 0;
    // //        else
    ...
    //             if (oGridDataAccessor(pGrid,yIndex, xIndexCenter, 1) >
    //             HEIGHT_THRESH ||
    //                     oGridDataAccessor(pGrid,yIndex, xIndexRightCorn, 1) >
    //                     HEIGHT_THRESH ||
    //                     oGridDataAccessor(pGrid,yIndex, xIndexLeftCorn, 1) >
    //                     HEIGHT_THRESH)
    //             {
    //                 retDist = 0;
    //             }
    ...
    while (!done) {
      ...
      double frontRightX = centreX - (dist + robotLength / 2) * sinTheta +
                           bufferFromCenter * cosTheta;
      double frontRightY = centreY + (dist + robotLength / 2) * cosTheta +
                           bufferFromCenter * sinTheta;
      double frontLeftX = centreX - (dist + robotLength / 2) * sinTheta -
                          bufferFromCenter * cosTheta;
      double frontLeftY = centreY + (dist + robotLength / 2) * cosTheta -
                          bufferFromCenter * sinTheta;
      ...
      PointForCoord(pGrid, frontCenterY, frontCenterX, yIndexCenter,
                    xIndexCenter);
      PointForCoord(pGrid, frontRightY, frontRightX, yIndexRightCorn,
                    xIndexRightCorn);
      PointForCoord(pGrid, frontLeftY, frontLeftX, yIndexLeftCorn,
                    xIndexLeftCorn);
      ...
      if ((yIndexCenter >= (int)pGrid->dataDimension[0].size) ||
        ...
        // if (yIndexCenter >= 0){ odbg(frontCenterX,frontCenterY,
        // xIndexCenter,yIndexCenter,pGrid);} else {ROS_INFO("skip");}
        // if (yIndexRightCorn >= 0){ odbg(frontRightX,frontRightY,
        // xIndexRightCorn,yIndexRightCorn,pGrid);} else {ROS_INFO("skip");}
        // if (yIndexLeftCorn >= 0){ odbg(frontLeftX,frontLeftY,
        // xIndexLeftCorn,yIndexLeftCorn,pGrid);} else {ROS_INFO("skip");}
        if ((yIndexCenter >= 0 &&
             oGridDataAccessor(pGrid, yIndexCenter, xIndexCenter, 1) >
                 HEIGHT_THRESH) ||
            (yIndexRightCorn >= 0 &&
             oGridDataAccessor(pGrid, yIndexRightCorn, xIndexRightCorn, 1) >
                 HEIGHT_THRESH) ||
            (yIndexLeftCorn >= 0 &&
             oGridDataAccessor(pGrid, yIndexLeftCorn, xIndexLeftCorn, 1) >
                 HEIGHT_THRESH)) {
          retDist = dist;
          done = true;
          //        end
    ...
    PointForCoord(pGrid, 0 /*robotLength / 2*/, bufferFromCenter, yIndex,
                  xIndexLeftCorn);
    PointForCoord(pGrid, 0 /*robotLength / 2*/, -bufferFromCenter, yIndex,
                  xIndexRightCorn);
    double distance = -1.0;
    ...
    while (continueChecking) {
      if (yIndex >= pGrid->dataDimension[0].size) {
        ...
        odbg(0, yIndex * pGrid->header.gridResolution, xIndexCenter, yIndex,
             pGrid);
        odbg(-bufferFromCenter, yIndex * pGrid->header.gridResolution,
             xIndexLeftCorn, yIndex, pGrid);
        odbg(bufferFromCenter, yIndex * pGrid->header.gridResolution,
             xIndexRightCorn, yIndex, pGrid);
    ...
    while (!done) {
      ...
      //
      double frontCenterX =
          centreX + radius * cosTheta - robotLength / 2 * sinTheta;
      double frontCenterY =
          centreY + radius * sinTheta + robotLength / 2 * cosTheta;
      double frontRightX = centreX + (radius + bufferFromCenter) * cosTheta -
                           robotLength / 2 * sinTheta;
      double frontRightY = centreY + (radius + bufferFromCenter) * sinTheta +
                           robotLength / 2 * cosTheta;
...
//ROS_INFO("distTravelled=%f",distTravelled);
      int centerXIndex = 0;//round(frontCenterX/resolution +
size(OccupancyGrid,2)/2);
      int centerYIndex = 0;//round(frontCenterY/resolution + 1);
      int rightXIndex = 0;//round(frontRightX/resolution +
size(OccupancyGrid,2)/2);
      int rightYIndex = 0;//round(frontRightY/resolution + 1);
      int leftXIndex = 0;//round(frontLeftX/resolution +
size(OccupancyGrid,2)/2);
      int leftYIndex = 0;//round(frontLeftY/resolution + 1);

      PointForCoord(pGrid, frontCenterY,frontCenterX, centerYIndex,
centerXIndex);
...
if ((oGridDataAccessor(pGrid,centerYIndex,centerXIndex,1) > HEIGHT_THRESH) ||
              (oGridDataAccessor(pGrid,rightYIndex,rightXIndex,1) >
HEIGHT_THRESH) ||
              (oGridDataAccessor(pGrid,leftYIndex,leftXIndex,1) >
HEIGHT_THRESH))
      {
          retDist = distTravelled - timestep * v;
          done = true;
...
*/
      ...
      eTraversableResult leftResult =
          IsPointTraversable(pGrid, frontLeftY, frontLeftX);
      eTraversableResult rightResult =
          IsPointTraversable(pGrid, frontRightY, frontRightX);
      eTraversableResult centerResult =
          IsPointTraversable(pGrid, frontCenterY, frontCenterX);
