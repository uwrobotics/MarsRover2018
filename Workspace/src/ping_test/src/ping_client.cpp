...
int main(int argc, char **argv) {
    ...
    ros::Publisher pingPub=nh.advertise<std_msgs::UInt8MultiArray>(PING_REQUEST_TOPIC, 1);
    ros::Subscriber sub=nh.subscribe(PING_REPLY_TOPIC, 1, replyCallback);
    ros::Publisher resultPub=nh.advertise<std_msgs::Bool>(PING_CLIENT_RESULT_TOPIC,1);
    ...
    while (ros::ok())
    {
        ...
        cout << "received " << numSuccesses 
             << " out of " << PING_NUM_ATTEMPTS 
             << " successful replies\n";
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(PING_TIME_BETWEEN_TESTS));
    }
