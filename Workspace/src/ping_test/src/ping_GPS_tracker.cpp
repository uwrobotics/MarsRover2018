...
enum ePingState { UNSTARTED, CONNECTED, DISCONNECTED };
...
int main(int argc, char **argv) {
  ...
  ros::Publisher gpsPub =
      nh.advertise<sensor_msgs::NavSatFix>(GPS_TARGET_TOPIC, 1);
  ros::Subscriber PingResultSub =
      nh.subscribe(PING_CLIENT_RESULT_TOPIC, 1, PingResultCallback);
  ros::Subscriber gpsResultSub =
      nh.subscribe(GPS_SENSOR_TOPIC, 1, GPSMessageCallback);
