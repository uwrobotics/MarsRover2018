...
public:
  ...
  void start(unsigned int milliseconds,
    ...
    timerThread = std::thread(&Timer::timer_operation, this, milliseconds,
                              expiry_handler);
...
private:
  ...
  void
  timer_operation(unsigned int milliseconds,
                  void (*expiry_handler)(const boost::system::error_code &)) {
    std::unique_lock<std::mutex> lk(mutex);
    do {
      dl->expires_from_now(boost::posix_time::milliseconds(milliseconds));
      dl->async_wait(
          boost::bind(expiry_handler, boost::asio::placeholders::error));
      started = true;
      restart = false;
      lk.unlock();
      condvar.notify_all();
