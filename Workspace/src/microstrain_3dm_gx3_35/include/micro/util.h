...
void unmake16UnsignedInt(unsigned int val, Byte* high, Byte* low)
{
    *low = static_cast<Byte>(val);
    *high = static_cast<Byte>(val >> 8);
}

void unmake32UnsignedInt(unsigned int val, Byte* byte3, Byte* byte2, Byte* byte1, Byte* byte0)
{
...
void pack32BitFloats(float* dest, const Byte* source_bytes, int length, bool little_endian)
{
    int ii;
    int byte_ind = 0;
